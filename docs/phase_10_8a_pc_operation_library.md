...
9. metadata 条目已存在时不重复写入，返回 `already_exists`。
...
3. 如果当前 IMAGE_VALID 已匹配 request.image.identity：
   - 不写入
   - 返回 already_exists
4. 否则写 IMAGE_VALID
```
...
1. ensure_service_attached()
2. get_metadata_summary()
3. 检查当前 IMAGE_VALID 是否匹配 request.image_identity
4. 如果不匹配：
   - 不写入
   - 返回 IMAGE_VALID_REQUIRED
5. 如果当前 image 已有 BOOT_ATTEMPT：
   - 不写入
   - 返回 already_exists
6. 否则写 BOOT_ATTEMPT
```
...
2. get_metadata_summary()
3. 检查当前 IMAGE_VALID 是否匹配 request.image_identity
4. 检查当前 image 是否已有 BOOT_ATTEMPT
5. 如果 IMAGE_VALID 不匹配：
   - 不写入
   - 返回 IMAGE_VALID_REQUIRED
6. 如果没有 BOOT_ATTEMPT：
   - 不写入
...
31. append_boot_attempt 已存在时返回 already_exists。
