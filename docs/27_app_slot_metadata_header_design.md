...
`GET_METADATA_SUMMARY` is the normal parsed view. `MEMORY_READ` is a generic raw
memory-read primitive for diagnostics; it is not metadata-specific and does not
mutate metadata.
