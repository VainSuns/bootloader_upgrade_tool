...
static void BootAlgorithm_SetFrameError(BootAlgorithm *algorithm, uint16_t stage)
{
    BootErrorDetail_Clear(&algorithm->last_error);
    algorithm->last_error.operation = BOOT_ERR_OP_FRAME;
                            ...
                            const BootDeviceInfo *device_info)
{
    ...
    if ((device_info->protocol_ver != BOOT_PROTOCOL_VERSION) ||
        ...
        (device_info->max_data_words > device_info->max_payload_words))
    {
        return 0U;
    }
    algorithm->io = *io;
...
void BootAlgorithm_ProcessOne(BootAlgorithm *algorithm)
{
    ...
    {
        BootAlgorithm_SetFrameError(algorithm, BOOT_ERR_STAGE_PAYLOAD);
        BootAlgorithm_SendStatus(algorithm, BOOT_STATUS_BAD_PAYLOAD_CRC);
        return;
    }
    if (algorithm->request.protocol_ver != BOOT_PROTOCOL_VERSION)
    {
        BootAlgorithm_SetFrameError(algorithm, BOOT_ERR_STAGE_HEADER);
    ...
    switch (algorithm->request.command)
    {
        ...
        default:
            BootAlgorithm_SetFrameError(algorithm, BOOT_ERR_STAGE_STATE);
