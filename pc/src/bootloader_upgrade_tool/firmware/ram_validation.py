...
RAM_EXEC_RANGES = (
    AddressRange(0x008000, 0x00C000),
)
...
def validate_ram_firmware_image(image: FirmwareImage) -> None:
    ...
    if not _inside_any(image.entry_point, image.entry_point + 1, RAM_EXEC_RANGES):
        raise ValueError("RAM entry point is outside allowed executable RAM")
