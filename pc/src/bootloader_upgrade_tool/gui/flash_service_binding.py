...
class FlashServiceBinding(QObject):
    def __init__(
        ...
        self._last_resource_error: tuple[str | None, str | None] | None = None
    ...
    def _render(self, *, preparing: bool = False) -> bool:
        ...
        error = (state.error_code, state.error_message)
        if state.error_code and error != self._last_resource_error:
            self._show({
                "operation": "prepare_flash_service",
                "status": state.status.name,
                "error": {"code": state.error_code, "message": state.error_message},
            })
        self._last_resource_error = error if state.error_code else None
