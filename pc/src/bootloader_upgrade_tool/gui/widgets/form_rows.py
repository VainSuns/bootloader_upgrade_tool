...
class PathFieldRow(QWidget):
    ...
    def __init__(
        ...
        *,
        ...
        self.browse_button.setToolTip(f"Select {label}")
        self.browse_button.setAccessibleName(f"Select {label}")
        self.browse_button.setIcon(manager.icon(semantic_icon, size=18))
        self.browse_button.setIconSize(QSize(18, 18))
        self.browse_button.setToolButtonStyle(Qt.ToolButtonStyle.ToolButtonIconOnly)
        button_width = max(40, SETTINGS_BROWSE_BUTTON_SIZE[0])
        button_height = max(INPUT_MINIMUM_HEIGHT, SETTINGS_BROWSE_BUTTON_SIZE[1])
        self.browse_button.setFixedSize(button_width, button_height)
        set_ui_variant(self.browse_button, "secondary")
        self.browse_button.setProperty("filePickerButton", True)
