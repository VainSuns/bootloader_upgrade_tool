...
class _CleanupRequest:
    def __init__(self, title="Release Connection", step_id="release"): self.title=title; self.step_id=step_id
    def create_plan(self,task_id):
        return TaskPlan(task_id,self.title,(TaskStepPlan(self.step_id,self.title,ProgressMode.INDETERMINATE),),TaskConnectionRequirement.CONNECTED,False,CompletionPolicy.REQUIRE_ACKNOWLEDGEMENT)
...
class GuiController(QObject):
    ...
    def request_task(self, request):
        ...
        if self._active: return self._reject(RequestRejectionCode.TASK_ALREADY_ACTIVE,"A task is already active")
        task_id=uuid4().hex
        try: plan=request.create_plan(task_id)
    ...
    def _prepare_generation(self):
        ...
        thread=QThread(); worker=TaskWorker(a.task_id,a.generation,job,a.cancellation,a.plan.cancellable)
    ...
    def request_cancel(self,task_id):
        ...
        if a.state.phase not in (TaskPhase.PENDING,TaskPhase.RUNNING,TaskPhase.CANCELLING) or a.state.disposition_state is not TaskDispositionState.NONE:
            return CancelRequestResult(False,task_id,rejection=RequestRejection(RequestRejectionCode.INVALID_RUNTIME_STATE,"Task is not executing",task_id))
    ...
    def _on_thread_finished(self,generation):
        ...
        a.thread_finished=True
        if a.thread is not None:
            retired=(a.thread,a.worker); self._retired_qobjects.append(retired); QTimer.singleShot(1000,lambda r=retired:self._retired_qobjects.remove(r) if r in self._retired_qobjects else None)
        a.thread=None; a.worker=None
        if a.fatal_error: return self._finalize_fatal()
    ...
    def _finish_connect(self,result):
        ...
        else: self._complete(result,RuntimeState.DISCONNECTED,connection_info=None,connection_suspect=False,last_error=None)
    def _finish_disconnect(self,cleanup):
        ...
        if primary.status in (TaskFinalStatus.SUCCEEDED,TaskFinalStatus.COMPLETED_AFTER_CANCEL_REQUEST):
            warning=None
            if cleanup.status is not TaskFinalStatus.SUCCEEDED:
                warning=GuiTaskWarning("CONNECTION_RELEASE_FAILED","Operation succeeded, but connection cleanup failed","cleanup",{"cleanup_error_code":cleanup.error.code if cleanup.error else "CLEANUP_FAILED"})
            final=replace(primary,step_results=evidence,warning=warning)
        else:
            error=primary.error
        ...
        self._complete(final,RuntimeState.DISCONNECTED,connection_info=None,connection_suspect=False,last_error=None)
    def _finish_task(self,result):
        ...
        if result.error and result.error.disposition is ErrorDisposition.FORCE_DISCONNECTED: return self._complete(result,RuntimeState.DISCONNECTED,connection_info=None,connection_suspect=False,last_error=result.error)
    ...
    def respond_task_action(self,task_id,action):
        ...
        elif action is TaskDialogAction.RETRY_CLEANUP:
            a.generation+=1; a.kind=_Kind.SHUTDOWN; a.request=_CleanupRequest("Shutdown","shutdown"); a.plan=a.request.create_plan(a.task_id); a.actions.clear()
            a.state=TaskState(a.task_id,a.plan,TaskPhase.PENDING,started_at=a.state.started_at); self.taskStateChanged.emit(a.state); self._run_generation()
    ...
    def _complete(self,result,state,**snapshot_changes):
        ...
        self._active=None
        changes={"state":state,"active_task_id":None,"disconnect_decision_pending":False}; changes.update(snapshot_changes)
        new=replace(self._snapshot,**changes); changed=new!=self._snapshot; self._snapshot=new
