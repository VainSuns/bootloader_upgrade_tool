...
def test_policy_preview_reasons() -> None:
    ...
    assert reason(make_summary()) == "FIRST_BOOT_REQUIRES_BOOT_ATTEMPT_WRITE"
...
def test_format_text_contains_pass_and_preview() -> None:
    ...
    assert "NOT_CHECKED_IN_PHASE_10_6_A" in text


def test_json_formatting() -> None:
    result = boot_status_probe.BootStatusResult(
        make_summary(),
        boot_status_probe.BootPolicyPreview(False, "FIRST_BOOT_REQUIRES_BOOT_ATTEMPT_WRITE"),
    )
    data = json.loads(boot_status_probe.format_json(result))
    assert data["preview"]["automatic_boot_allowed"] is False
    assert data["preview"]["reason"] == "FIRST_BOOT_REQUIRES_BOOT_ATTEMPT_WRITE"
...
def test_collect_boot_status_is_read_only() -> None:
    ...
    assert result.preview.reason == "FIRST_BOOT_REQUIRES_BOOT_ATTEMPT_WRITE"
